...
Our goal here is to introduce the LIMA language. We are not, however, providing a comprehensive overview of the language, but we present the major elements. In particular, we elide the portion
...
\y{atomX}. The code is mostly generated in static single-assignment
...
Certification (e.g., DO178-C~\cite{do178c}) of auto-generated code has
...
system execution by calling another special function \y{printProbe}. The
utility function \y{mapM\_} is standard Haskell which maps a function
(\y{printProbes}) over a container of values (\y{probes}). In Figure
